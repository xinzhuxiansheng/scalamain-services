...
object Parallelism extends ZIOAppDefault {
  ...
  val meaningOfLift = ZIO.succeed(42)
  val favLang = ZIO.succeed("Scala")
  val combined = meaningOfLift.zip(favLang) // combines/zips ina sequential

  // combine in parallel
  val combinedPar = meaningOfLift.zipPar(favLang) // combination is parallel
  ...
  */
  ...
  /*
    ...
    - if the entire thing is inteerupted => all effects are interrupted
  */
  ...
  def wordCountParallel(n: Int): UIO[Int] = {
    ...
    // v3 - reduceAllPar
    ZIO.reduceAll(ZIO.succeed(0), effects)(_ + _)
  }
