...
object Fibers extends ZIOAppDefault {
  ...
  def runOnAnotherThread_v2[R, E, A](zio: ZIO[R, E, A]) = for {
    ...
    case Exit.Failure(value) => s"failed with $value"
  }
  ...
  // spawn n fibers, count the n for words in each file
  ...
  // part 1 - an effect which reads on file and counts the words there
  def countWords(path: String): UIO[Int] = ZIO.succeed {
  ...
  def wordCountParallel(n: Int): UIO[Int] = {
    val effect: Seq[ZIO[Any, Nothing, Int]] = (1 to n)
      ...
      .map((fiberEff: ZIO[Any, Nothing, Fiber[Nothing, Int]]) => fiberEff.flatMap(_.join)) // list of effects returing values

    effect.reduce { (zioa, ziob) =>
      for {
        a <- zioa
        b <- ziob
      } yield a + b
  ...
  //  def run = runOnAnotherThread_v2(meaningOfLife).debugThread
